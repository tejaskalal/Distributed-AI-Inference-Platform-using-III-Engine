...
describe('Hexastore', function() {
  ...
  it('import export to string', function() {
    hexastore.add(['a', 'b', 'c']);
    hexastore.add(['a', 'x', 'y', true]);
    hexastore.add(['b', 'x', 'y', true]);
    var str = hexastore.exportString();
    var copyHexastore = new H();
    copyHexastore.importString(str);
    var res = copyHexastore.queryXXX([null, null, null]);
    expect(res.length).toEqual(3);
    expect(res).toContain(['a', 'b', 'c', true]);
    expect(res).toContain(['a', 'x', 'y', true]);
    expect(res).toContain(['b', 'x', 'y', true]);
